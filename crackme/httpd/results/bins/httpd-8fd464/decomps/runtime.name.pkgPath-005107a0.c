...
/* DWARF original prototype: void runtime.name.pkgPath(runtime.name n, string ~r0) */

string runtime_name_pkgPath(runtime_name n)

{
  ...
  runtime_name local_18;
  uint local_c;
  
                    /* Unresolved local var: int l@[???]
                       Unresolved local var: int off@[???]
                       Unresolved local var: runtime.nameOff nameOff@[???] */
  ...
  if ((n_spill.bytes != (uint8 *)0x0) && ((*n_spill.bytes & 4) != 0)) {
    ...
    uVar2 = local_c;
    runtime_resolveNameOff((void *)(ulong)local_c,(runtime_nameOff)lVar4);
    sVar7 = runtime_name_name(local_18);
    sVar8.len = sVar7.len;
