...
/* DWARF original prototype: void syscall.getpeername(int fd, syscall.RawSockaddrAny * rsa,
   syscall._Socklen * addrlen, error err) */

error syscall_getpeername(int fd,syscall_RawSockaddrAny *rsa,syscall__Socklen *addrlen)

{
  ...
  if (local_20 == 0) {
    ...
    if (local_20 < 3) {
      ...
      if (local_20 == 2) goto LAB_00547ee7;
    }
    else {
