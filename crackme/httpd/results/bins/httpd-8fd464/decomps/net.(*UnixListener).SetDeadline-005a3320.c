...
/* DWARF original prototype: void net.(*UnixListener).SetDeadline(net.UnixListener * l, time.Time t,
   error ~r1) */

error net___UnixListener__SetDeadline(net_UnixListener *l,time_Time t)

{
  ...
  if ((l_spill != (net_UnixListener *)0x0) && (l_spill->fd != (net_netFD *)0x0)) {
    ...
    if (local_28 != (runtime_interfacetype *)0x0) {
      ...
      *(int *)(t_spill.wall + 0x18) = (l_spill->fd->net).len;
      ...
      *(runtime_itab **)&((runtime_itab *)(t_spill.wall + 0x20))->hash = (l_spill->fd->laddr).tab;
      if (runtime_writeBarrier._0_4_ == 0) {
