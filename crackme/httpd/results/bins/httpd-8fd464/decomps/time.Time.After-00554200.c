...
/* DWARF original prototype: void time.Time.After(time.Time t, time.Time u, bool ~r1) */
...

{
  ...
  if (u_spill.ext < t_spill.ext) {
    bVar1 = true;
  }
  else if (u_spill.ext == t_spill.ext) {
    bVar1 = ((uint)u_spill.wall & 0x3fffffff) < ((uint)t_spill.wall & 0x3fffffff);
  }
  else {
    bVar1 = false;
  }
