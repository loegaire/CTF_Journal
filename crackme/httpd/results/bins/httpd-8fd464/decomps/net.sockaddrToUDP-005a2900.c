...
/* DWARF original prototype: void net.sockaddrToUDP(syscall.Sockaddr sa, net.Addr ~r1) */

net_Addr net_sockaddrToUDP(syscall_Sockaddr sa)

{
  uint uVar1;
  syscall_SockaddrInet4 *extraout_RAX;
  syscall_SockaddrInet4 *extraout_RAX_00;
  runtime_itab *prVar2;
  long *plVar3;
  long in_FS_OFFSET;
  net_Addr nVar4;
  ...
  if (in_stack_00000008 != (runtime_itab *)0x0) {
    ...
    if (uVar1 == 0x61a1523f) {
      if (in_stack_00000008 == (runtime_itab *)go_itab__syscall_SockaddrInet4_syscall_Sockaddr) {
        ...
        local_30[3] = sa_1->Port;
        nVar4.tab = (runtime_itab *)go_itab__net_UDPAddr_net_Addr;
        return nVar4;
      }
    }
    else {
      ...
      if ((uVar1 == 0x98d3a9ba) &&
        ...
        if (runtime_writeBarrier._0_4_ == 0) {
          *plVar3 = (long)&sa_1->raw;
          ...
          runtime_gcWriteBarrierCX();
          sa_1 = extraout_RAX_00;
        }
        plVar3[3] = sa_1->Port;
        plVar3[5] = local_20;
        if (runtime_writeBarrier._0_4_ == 0) {
          plVar3[4] = (long)local_28;
        }
        else {
          runtime_gcWriteBarrierDX();
        }
        nVar6.tab = (runtime_itab *)go_itab__net_UDPAddr_net_Addr;
        return nVar6;
      }
    }
  }
