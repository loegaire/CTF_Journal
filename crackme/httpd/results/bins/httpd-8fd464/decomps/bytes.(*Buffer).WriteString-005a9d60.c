...
/* DWARF original prototype: void bytes.(*Buffer).WriteString(bytes.Buffer * b, string s, int n,
   error err) */

multireturn_int_error_ bytes___Buffer__WriteString(bytes_Buffer *b,string s)

{
  ...
  if ((long)(uVar2 - uVar1) < s_spill.len) {
    bytes___Buffer__grow(b,(int)b_spill);
  }
  else {
    if (uVar2 < uVar1 + s_spill.len) {
                    /* WARNING: Subroutine does not return */
      runtime_panicSliceAcap();
    }
    (b_spill->buf).len = uVar1 + s_spill.len;
    local_18 = uVar1;
  }
  uVar1 = (b_spill->buf).len;
