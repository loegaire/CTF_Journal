...
/* DWARF original prototype: void bytes.(*Reader).Read(bytes.Reader * r, []uint8 b, int n, error
   err) */

multireturn_int_error_ bytes___Reader__Read(bytes_Reader *r,__uint8 b)

{
  ...
  if (r_spill->i < (r_spill->s).len) {
    ...
    if (uVar3 <= uVar1) {
      ...
      puVar4 = puVar2 + (uVar3 & (long)(uVar3 - (r_spill->s).cap) >> 0x3f);
