...
def k_from_bytes(data: bytes, endian: str, bit_order: str) -> int:
    if bit_order == "msb":
        bits = "".join(f"{b:08b}" for b in (data if endian == "big" else data[::-1]))
    else:
        bits = "".join(f"{b:08b}"[::-1] for b in (data if endian == "big" else data[::-1]))
    return int(bits, 2)
...
for endian in ("big", "little"):
    for bit_order in ("msb", "lsb"):
        k = k_from_bytes(md5, endian, bit_order)
        for forward in (True, False):
            match = O1 == perm_pow(T, k, forward)
            if match:
                print("match", endian, bit_order, "forward" if forward else "backward")
